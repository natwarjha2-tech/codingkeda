...
function LoginForm() {
  ...
  const [otpMsgType, setOtpMsgType] = useState<"error" | "success">("error");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else if (flow === "free") { localStorage.removeItem("isNewUser"); router.replace("/dashboard"); }
      else if (flow === "survey") { localStorage.removeItem("isNewUser"); router.replace("/recommendation"); }
      else if (isNewUser && !surveyDone) { localStorage.removeItem("isNewUser"); router.replace("/"); setTimeout(() => { document.getElementById("survey")?.scrollIntoView({ behavior: "smooth" }); }, 400); }
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const flow = searchParams.get("flow") || "";
      if (flow === "free") { router.replace("/login?flow=free"); }
      else if (flow === "survey") { router.replace("/login?flow=survey"); }
      else { localStorage.setItem("isNewUser", "true"); setIsSignup(false); }
  ...
  return (
    <>
      <Navbar />
      <main className="h-screen bg-white pt-16 overflow-hidden">
        <div className="flex h-[calc(100vh-64px)]">
          ...
          <div className="w-full lg:w-1/2 flex items-center justify-center px-6 py-8 bg-white overflow-hidden">
            <div className="w-full max-w-md">
              {/* Mobile logo */}
              <div className="lg:hidden mb-6 text-center">
                <Link href="/" className="text-2xl font-extrabold text-slate-900">Coding<span className="text-purple-600">Kida</span></Link>
              </div>
                ...
                {!isSignup ? (
                  ...
                  <motion.div key="login" initial={{ opacity: 0, rotateY: -90 }} animate={{ opacity: 1, rotateY: 0 }} exit={{ opacity: 0, rotateY: 90 }} transition={{ duration: 0.4, ease: "easeInOut" }}>
                    <div className="bg-white rounded-2xl p-8 lg:p-10 shadow-xl border border-slate-100">
                      ...
                      <div className="flex items-center gap-3 my-4">
                        ...
                        <span className="text-slate-400 text-xs font-medium">or login with email OTP</span>
                        <div className="flex-1 h-px bg-slate-200" />
                      </div>
                      ...
                      <div>
                        {!otpSent ? (
                          <div className="space-y-3">
                            <input type="email" placeholder="Enter your email for OTP" value={otpEmail} onChange={e => setOtpEmail(e.target.value)}
                              className="w-full bg-slate-50 border border-slate-200 focus:border-purple-500 rounded-xl px-4 py-3 text-slate-900 text-sm outline-none transition-colors placeholder:text-slate-400" />
                            {otpMsg && <p className={`text-xs px-3 py-2 rounded-lg ${otpMsgType === 'error' ? 'text-red-500 bg-red-50' : 'text-green-600 bg-green-50'}`}>{otpMsg}</p>}
                            <button type="button" disabled={otpLoading} onClick={async () => {
                              if (!otpEmail.trim() || !otpEmail.includes('@')) { setOtpMsg('Please enter a valid email.'); setOtpMsgType('error'); return; }
                              setOtpLoading(true); setOtpMsg('');
                              try {
                                const res = await fetch('/api/auth/send-otp', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: otpEmail.trim() }) });
                                const data = await res.json();
                                if (data.success) { setOtpSent(true); setOtpMsg('OTP sent to ' + otpEmail + '. Check your inbox.'); setOtpMsgType('success'); }
                                else { setOtpMsg(data.message || 'Failed to send OTP.'); setOtpMsgType('error'); }
                              } catch { setOtpMsg('Network error.'); setOtpMsgType('error'); }
                              setOtpLoading(false);
                            }}
                              className="w-full flex items-center justify-center gap-2 py-3 rounded-xl font-semibold text-purple-600 text-sm bg-purple-50 hover:bg-purple-100 border border-purple-200 transition-all">
                              {otpLoading ? 'Sending...' : '📧 Send OTP'}
                            </button>
                          </div>
                        ) : (
                          <div className="space-y-3">
                            <p className="text-xs text-green-600 bg-green-50 px-3 py-2 rounded-lg">OTP sent to {otpEmail}</p>
                            <input type="text" placeholder="Enter 6-digit OTP" maxLength={6} value={otpCode} onChange={e => setOtpCode(e.target.value)}
                              className="w-full bg-slate-50 border border-slate-200 focus:border-purple-500 rounded-xl px-4 py-3 text-slate-900 text-sm outline-none text-center tracking-[8px] font-bold text-lg" />
                            {otpMsg && otpMsgType === 'error' && <p className="text-xs text-red-500 bg-red-50 px-3 py-2 rounded-lg">{otpMsg}</p>}
                            <button type="button" disabled={otpLoading} onClick={async () => {
                              if (!otpCode.trim() || otpCode.length !== 6) { setOtpMsg('Enter the 6-digit code.'); setOtpMsgType('error'); return; }
                              setOtpLoading(true); setOtpMsg('');
                              try {
                                const res = await fetch('/api/auth/verify-otp', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: otpEmail.trim(), otp: otpCode.trim() }) });
                                const data = await res.json();
                                if (data.success && data.token) {
                                  localStorage.setItem('token', data.token);
                                  localStorage.setItem('userEmail', data.user.email || '');
                                  localStorage.setItem('user', JSON.stringify({ ...data.user, loginMethod: 'otp' }));
                                  window.location.href = '/dashboard';
                                } else { setOtpMsg(data.message || 'Invalid OTP.'); setOtpMsgType('error'); }
                              } catch { setOtpMsg('Network error.'); setOtpMsgType('error'); }
                              setOtpLoading(false);
                            }}
                              className="w-full flex items-center justify-center gap-2 py-3 rounded-xl font-semibold text-white text-sm bg-purple-600 hover:bg-purple-700 transition-all">
                              {otpLoading ? 'Verifying...' : '✓ Verify OTP'}
                            </button>
                            <button type="button" onClick={() => { setOtpSent(false); setOtpCode(''); setOtpMsg(''); }}
                  ...
                  <motion.div key="signup" initial={{ opacity: 0, rotateY: 90 }} animate={{ opacity: 1, rotateY: 0 }} exit={{ opacity: 0, rotateY: -90 }} transition={{ duration: 0.4, ease: "easeInOut" }}>
                    <div className="bg-white rounded-2xl p-8 lg:p-10 shadow-xl border border-slate-100">
                      ...
                      <div className="flex items-center gap-3 my-4">
                        <div className="flex-1 h-px bg-slate-200" />
                        <span className="text-slate-400 text-xs font-medium">or sign up with email OTP</span>
                        <div className="flex-1 h-px bg-slate-200" />
                      </div>

                      {/* OTP Signup */}
                      <button type="button" onClick={() => { setIsSignup(false); setOtpEmail(''); setOtpSent(false); setOtpCode(''); }}
                        className="w-full flex items-center justify-center gap-2 py-3 rounded-xl font-semibold text-purple-600 text-sm bg-purple-50 hover:bg-purple-100 border border-purple-200 transition-all">
        ...
        {/* Footer */}
        <div className="border-t border-slate-100 py-4 px-6 flex items-center justify-between text-xs text-slate-400">
          <span>© 2024 CodingKida. All rights reserved.</span>
